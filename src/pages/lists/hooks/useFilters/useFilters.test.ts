...
describe('useFilters', () => {
  ...
  it('should set a Created by filter and expose it via createdByFilter', () => {
    ...
    result.current.setUserFilter(CREATED_BY_PREFIX, 'user-1', 'Doe, John');
    ...
    expect(result.current.createdByFilter).toEqual({ userId: 'user-1', userName: 'Doe, John' });
    expect(result.current.updatedByFilter).toEqual({ userId: '', userName: '' });
  });
  ...
  it('should replace a previously selected Created by user when a new one is selected', () => {
    ...
    result.current.setUserFilter(CREATED_BY_PREFIX, 'user-2', 'Smith, Jane');
  ...
  it('should track Created by and Updated by filters independently', () => {
    ...
    result.current.setUserFilter(UPDATED_BY_PREFIX, 'user-2', 'Smith, Jane');
    ...
    expect(result.current.createdByFilter).toEqual({ userId: 'user-1', userName: 'Doe, John' });
    expect(result.current.updatedByFilter).toEqual({ userId: 'user-2', userName: 'Smith, Jane' });
  });

  it('should clear a Created by filter and its cached display name', () => {
    const { result, rerender } = renderHook(() => useFilters());

    result.current.setUserFilter(CREATED_BY_PREFIX, 'user-1', 'Doe, John');
    locationSearch = '?filters=created_by.user-1';
    rerender();
    expect(result.current.createdByFilter.userName).toBe('Doe, John');
    ...
    expect(result.current.createdByFilter).toEqual({ userId: '', userName: '' });
  });
});
